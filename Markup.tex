...
%DIF DEL EcoGEnIE.v1.tex   Fri Jul  6 10:01:00 2018
%DIF ADD EcoGEnIE.tex      Fri Jul  6 10:20:23 2018
%% ---------------------------------
...
\title{EcoGEnIE \DIFdelbegin \DIFdel{0.1}\DIFdelend \DIFaddbegin \DIFadd{0.2}\DIFaddend : Plankton Ecology in the \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend Earth system model}


\DIFdelbegin %DIFDELCMD < \Author[1,2]{}{Ben~A.~Ward}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \Author[1,2,3]{}{Ben~A.~Ward}
\DIFaddend \Author[1]{}{Jamie~D.~Wilson}
\Author[1]{}{Ros~M.~Death}
\Author[1]{}{Fanny~M.~Monteiro}
\Author[2]{}{Andrew~Yool}
\DIFdelbegin %DIFDELCMD < \Author[1,3]{}{Andy~Ridgwell}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \Author[1,4]{}{Andy~Ridgwell}
\DIFaddend 

\affil[1]{\small{School of Geographical Sciences, University of Bristol, Bristol BS8 1SS, UK}}
\DIFdelbegin %DIFDELCMD < \affil[2]{\small{National Oceanography Centre, European Way, Southampton SO14 3ZH, UK}}
%DIFDELCMD < \affil[3]{\small{Department of Earth Sciences, University of California, Riverside CA, USA}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \affil[2]{\small{Ocean and Earth Science, University of Southampton, National Oceanography Centre, Southampton, UK}}
...
\section{The \DIFdelbegin \DIFdel{GENIE}\DIFdelend \DIFaddbegin \DIFadd{GEnIE}\DIFaddend /\DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend Earth system model}\DIFdelbegin %DIFDELCMD < \label{GENIE}
%DIFDELCMD < %%%
\DIFdel{GENIE }\DIFdelend \DIFaddbegin \label{GEnIE}
\DIFadd{GEnIE }\DIFaddend is an `Earth system model of intermediate complexity' (EMIC) \citep{Claussen:2002} and is based on a \DIFdelbegin \DIFdel{modularised }\DIFdelend \DIFaddbegin \DIFadd{modular }\DIFaddend framework that allows different components of the Earth system, including ocean circulation, ocean biogeochemistry, deep-sea sediments and geochemistry, to be incorporated \citep{Lenton:2007b}. The simplified atmosphere and carbon-centric version of \DIFdelbegin \DIFdel{GENIE }\DIFdelend \DIFaddbegin \DIFadd{GEnIE }\DIFaddend we use --~\DIFdelbegin \DIFdel{cGENIE}\DIFdelend \DIFaddbegin \DIFadd{cGEnIE}\DIFaddend ~-- has been previously applied to explore and understand the interactions between biological productivity, biogeochemistry and climate over a range of timescales and time periods \citep[e.g.,][]{Ridgwell:2010,Monteiro:2012,Norris:2013,John:2014,Gibbs:2015,Meyer:2016,Tagliabue:2016}.  As is common for EMICs, \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend features a decreased spatial and temporal resolution in order to facilitate the efficient simulation of the various interacting components. This imposes limits on the resolution of ecosystem dynamics to large-scale annual/seasonal patterns in contrast to higher resolutions often used to model modern ecosystems. However, our motivation for incorporating a new marine ecosystem module into \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend is to focus on the explicit interactions between ecosystems, biogeochemistry and climate that are computationally prohibitive in higher resolution models. In other words, our motivation is to include and explore a more complete range of interactions and dynamics within the marine system, at the expense of spatial fidelity and with \DIFdelbegin \DIFdel{a greater }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend intention to explore long timescale and paleoceanographic questions, rather than short-term and future anthropogenic concerns. 
...
\DIFdelbegin \DIFdel{ECOGEM }\DIFdelend \DIFaddbegin \DIFadd{In both BIOGEM and ECOGEM,  the incoming shortwave solar radiation intensity is taken from the climate component in cGEnIE and varies seasonally \mbox{%DIFAUXCMD
\citep{Edwards:2005, Marsh:2011}}\hspace{0pt}%DIFAUXCMD
. However, ECOGEM }\DIFaddend uses a slightly more complex light attenuation scheme than BIOGEM, which simply calculates a mean solar (shortwave) irradiance averaged over the depth of the surface layer, \DIFdelbegin \DIFdel{and assuming a length }\DIFdelend \DIFaddbegin \DIFadd{assuming a clear-water light attenuation }\DIFaddend scale of 20 m \DIFdelbegin \DIFdel{over which light decays \mbox{%DIFAUXCMD
\citep{Doney:2006b}}\hspace{0pt}%DIFAUXCMD
. 
BIOGEM then takes this mean irradiance and applies a Michaelis-Menten like limitation term, assuming a half saturation value for light of 20 W m$^{-2}$ \mbox{%DIFAUXCMD
\citep{Doney:2006b}}\hspace{0pt}%DIFAUXCMD
. At the ocean surface, the incoming shortwave solar radiation intensity is taken from the climate component in cGENIE and varies seasonally \mbox{%DIFAUXCMD
\citep{Edwards:2005, Marsh:2011}}\hspace{0pt}%DIFAUXCMD
. 
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Doney:2006b}}\hspace{0pt}%DIFAUXCMD
...
\DIFdelend %DIF > \subsubsection{Respiration}
...
}\DIFdelend %DIF > A linear respiration rate is applied to degrade plankton carbon biomass into dissolved inorganic carbon. This is achieved through a $J$ by $I_r$ respiration matrix, $\matr{r}$, which is non-zero only for $i_r={\rm DIC}$.
...
%
\begin{align}
\frac{\partial {R_{i_r}}}{\partial t} = & \sum_{j=1}^{J} \DIFdelbegin %DIFDELCMD < \Big( %%%
\DIFdelend \underbrace{-V_{j,i_r} \cdot B_{j,{\rm C}} }_{\rm uptake}
 \DIFdelbegin \DIFdel{+ }%DIFDELCMD < \underbrace{r_{j,i_r} \cdot B_{j,{\rm C}} }%%%
\DIFdel{_{\rm respiration} }%DIFDELCMD < \Big) %%%
...
\frac{\partial B_{j,i_b}}{\partial t} = &+ \underbrace{V_{j,i_b} \cdot B_{j,C}}_{\rm uptake} 
 - \underbrace{m_{j} \cdot B_{j,i_b}}_{\rm basal~mortality} %DIF <  
 \DIFdelbegin \DIFdel{- }%DIFDELCMD < \underbrace{r_{i_b,j} \cdot B_{C,j}}%%%
\DIFdel{_{\rm respiration} }%DIFDELCMD < \nonumber\\ 
%DIFDELCMD <  &%%%
...
\end{equation}
%
\DIFdelbegin \DIFdel{Dissolved organic matter }\DIFdelend \DIFaddbegin \DIFadd{The dissolved organic matter vector }\DIFaddend (${\matr D}_{1}$) \DIFdelbegin \DIFdel{is an explicit tracer that is }\DIFdelend \DIFaddbegin \DIFadd{includes three explicit tracers that are }\DIFaddend transported by the ocean circulation model and \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend degraded back to \DIFdelbegin \DIFdel{its }\DIFdelend \DIFaddbegin \DIFadd{their }\DIFaddend constituent nutrients with a fixed turnover time of $\lambda$ (= 0.5 years). Particulate organic matter (POM) is not represented \DIFdelbegin \DIFdel{as an explicit state variable }\DIFdelend \DIFaddbegin \DIFadd{with explicit state variables }\DIFaddend in either ECOGEM or BIOGEM. Instead, its implicit production in the surface layer \DIFaddbegin \DIFadd{(and the corresponding export below the surface layer) }\DIFaddend is given by 
%
\begin{equation} 
...
%DIF > & Plankton respiration 	& $r_{i_b={\rm DIC}}$ & 0.05 & d$^{-1}$ \\ 
& 					\DIFdelbeginFL \DIFdelFL{Plankton respiration 	}\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{$r_{i_b={\rm DIC}}$ }%DIFDELCMD < & %%%
\DIFdelFL{0.05 }%DIFDELCMD < & %%%
\DIFdelFL{d$^{-1}$ }%DIFDELCMD < \\ 
%DIFDELCMD < & 					& %%%
\DIFdelendFL $r_{i_b\neq{\rm DIC}}$ & 0 & d$^{-1}$ \\ 
...
As far as possible, the parameter values applied in ECOGEM were kept as close as possible to previously published versions of the model \citep{Ward:2016}. There were however a few modifications that were required to bring \DIFdelbegin \DIFdel{EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE }\DIFaddend into first order agreement with observations and the current version of \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend \citep{Ridgwell:prep}. In particular, in comparison to the biogeochemical model used in \citet{Ward:2016}, the amount of soluble iron supplied to \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend by atmospheric deposition is considerably less. With a smaller source of iron, it was necessary to decrease the iron demand of the plankton community, and this was achieved by decreasing $Q^{\rm max}_{\rm Fe}$ and $Q^{\rm min}_{\rm Fe}$ by five-fold ($Q^{\rm max}_{\rm Fe}$ from 20 to 4 nmol Fe (mmol C)$^{-1}$, and $Q^{\rm min}_{\rm Fe}$ from 5 to 1 nmol Fe (mmol C)$^{-1}$).

We also found that the flexible stoichiometry of ECOGEM led to excessive export of carbon from the surface ocean, attributable to higher C:P ratios in organic matter (BIOGEM assumes a Redfieldian C:P of 106). This effect was moderated by \DIFdelbegin \DIFdel{adding the respiration term, which returns a fraction of carbon biomass directly to DIC (it is assumed that other elements are not lost in this way)}\DIFdelend \DIFaddbegin \DIFadd{increasing the size of the minimum phosphate:carbon quota, $Q^{\rm min}_{\rm P}$ \mbox{%DIFAUXCMD
\citep[relative to][]{Ward:2012}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The additional production of POC also led to increased production of calcium carbonate. This was counteracted by increasing the PIC:POC production ratio ($r^{\rm CaCO_3:POC}$) from 0.022 to 0.0285, and decreasing the thermodynamic calcification rate power ($\eta$) from 1.28 to 0.744 \citep{Ridgwell:2007}.
...
We ran \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend \citep[as configured and described in][]{Ridgwell:prep} and \DIFdelbegin \DIFdel{EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE }\DIFaddend (as described here) each for period of 10,000 years. These runs were initialised from a homogenous and static ocean, with an imposed constant atmospheric CO$_2$ concentration of 278 ppm. We present model output from the 10,000th year of integration. 

\subsection{Observations}

 Although they are not necessarily strictly comparable, we compare results from the pre-industrial configurations of \DIFdelbegin \DIFdel{cGENIE and EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE and EcoGEnIE }\DIFaddend to contemporary climatologies from a range of sources. Global climatologies of dissolved phosphate and oxygen are drawn from the World Ocean Atlas \DIFdelbegin \DIFdel{(WOA }\DIFdelend 2009 \DIFdelbegin \DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[WOA09 -][]{Garcia:2010}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , while DIC and alkalinity are taken from Global Ocean Data Analysis Project \DIFdelbegin \DIFdel{(GLODAP)}\DIFdelend \DIFaddbegin \DIFadd{version 2 \mbox{%DIFAUXCMD
\citep[GLODAPv2 -][]{Olsen:2016}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Surface chlorophyll concentrations represent a climatological average from 1997 to 2002, estimated by the SeaWiFS satellite. Depth-integrated primary production is from \citet{Behrenfeld:1997}. All of these interpolated global fields have been re-gridded onto the \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend 36\(\times\)36\(\times\)16 grid.

Observed dissolved iron concentrations are those published by Tagliabue et al. (2012). These data are too sparse and variable to allow reliable mapping on the \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend grid, and are therefore shown as individual data.
...
Annual mean global distributions are presented for the upper 80.8~m of the water column, corresponding to the model surface layer. In Figure~\ref{fig:nutrients} we compare output from the two models to observations of dissolved phosphate and iron. Surface phosphate concentrations are broadly similar between the two versions of the model, except that \DIFdelbegin \DIFdel{EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE }\DIFaddend provides slightly lower estimates in the Southern Ocean and equatorial upwellings. Both versions strongly underestimate surface phosphate in the equatorial and north Pacific, and to a lesser extent in the north and east Atlantic, the Arctic and the Arabian Sea. This is likely attributable in part to the model underestimating the strength of upwelling in these regions. It should also be noted that the observations may in some cases be unrepresentative of the true surface layer, when this is significantly shallower than 80.8 m. In such cases the observed value will be affected by measurements from below the surface layer. Iron distributions are also broadly similar between the two models, with \DIFdelbegin \DIFdel{EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE }\DIFaddend showing slightly lower iron concentrations over most of the ocean. 
...
Surface $\Delta$pCO$_2$ from the two models is shown in Figure~\ref{fig:dpCO2}. \DIFdelbegin \DIFdel{EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE }\DIFaddend shows weaker CO$_2$ outgassing in \DIFaddbegin \DIFadd{the }\DIFaddend tropical band, with a much stronger ocean-to-atmosphere flux in the Western Arctic. 

In Figure~\ref{fig:particulates} we show the annual mean rate of particulate organic matter production in the surface layer, and the relative differences between ECOGEM and BIOGEM. In comparison to \DIFdelbegin \DIFdel{cGENIE, EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE, EcoGEnIE }\DIFaddend shows elevated POC production in all regions. Production of CaCO$_3$ is globally less variable in \DIFdelbegin \DIFdel{EcoGENIE than cGENIE}\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE than cGEnIE}\DIFaddend , with notable higher fluxes in the oligotrophic gyres and polar regions. 

The relative proportions in which these elements and compounds are exported from the surface ocean are regulated by the stoichiometry of biological production. In \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend (BIOGEM), carbon and phosphorus production are rigidly coupled through a fixed ratio of 106:1, while POFe:POC and CaCO$_3$:POC \DIFdelbegin \DIFdel{production }\DIFdelend \DIFaddbegin \DIFadd{export flux }\DIFaddend ratios are regulated as a function of environmental conditions. In \DIFdelbegin \DIFdel{ecoGENIE }\DIFdelend \DIFaddbegin \DIFadd{ecoGEnIE }\DIFaddend (ECOGEM), phosphorus, iron and carbon \DIFdelbegin \DIFdel{production }\DIFdelend \DIFaddbegin \DIFadd{fluxes }\DIFaddend are all decoupled through the flexible quota physiology, which depends on both environmental conditions, and the status of the food-web. Only CaCO$_3$:POC \DIFdelbegin \DIFdel{production }\DIFdelend \DIFaddbegin \DIFadd{flux }\DIFaddend ratios are regulated via the same mechanism in the two models (although we decreased the average CaCO$_3$:POC ratio in ECOGEM to compensate for the elevated POC production relative to POP). 

\begin{figure}[htp]
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{PO$_4$ - WOA (2009)}
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/WOA_2009_data/surface_p_an.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/WOA_2009_data/surface_p_an.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{PO$_4$ - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/ocn_PO4.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/ocn_PO4.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{PO$_4$ - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_PO4.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_PO4.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_PO4_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_PO4_clrbr.png}
\DIFaddendFL \end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{dFe - \citet{Tagliabue:2012}}
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/Tagliabue_data/surface_fe.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/Tagliabue_data/surface_fe.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{dFe - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/ocn_TDFe.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/ocn_TDFe.png}
 \DIFaddendFL \label{fig:nutrients3}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{dFe - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_TDFe.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_TDFe.png}
 \DIFaddendFL \label{fig:nutrients4}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_TDFe_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_TDFe_clrbr.png}
\DIFaddendFL \end{subfigure}
\caption{Surface concentrations of dissolved \DIFdelbeginFL \DIFdelFL{inorganic nutrients}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{phosphate (mmol PO$_4$ m$^{-3}$) and iron (mmol dFe m$^{-3}$ )}\DIFaddendFL .}
\label{fig:nutrients}
\end{figure}
...
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{DIC - \DIFdelbeginFL \DIFdelFL{GLODAP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GLODAPv2}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/GLODAP_data/surface_TCO2.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/GLODAP_data/surface_TCO2.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{DIC -  - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/ocn_DIC.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/ocn_DIC.png}
 \DIFaddendFL \label{fig:carbon1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{DIC - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_DIC.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_DIC.png}
 \DIFaddendFL \label{fig:carbon2}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_DIC_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_DIC_clrbr.png}
\DIFaddendFL \end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Oxygen - WOA (2009)}
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/WOA_2009_data/surface_o_an.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/WOA_2009_data/surface_o_an.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Oxygen - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/ocn_O2.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/ocn_O2.png}
 \DIFaddendFL \label{fig:carbon3}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Oxygen - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_O2.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_O2.png}
 \DIFaddendFL \label{fig:carbon4}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_O2_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_O2_clrbr.png}
\DIFaddendFL \end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Alkalinity - \DIFdelbeginFL \DIFdelFL{GLODAP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GLODAPv2}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/GLODAP_data/surface_PALK.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/GLODAP_data/surface_TALK.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Alkalinity - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/ocn_ALK.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/ocn_ALK.png}
 \DIFaddendFL \label{fig:carbon3}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Alkalinity - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_ALK.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_ALK.png}
 \DIFaddendFL \label{fig:carbon4}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_ALK_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_ALK_clrbr.png}
\DIFaddendFL \end{subfigure}
\caption{Surface concentrations of dissolved inorganic carbon \DIFaddbeginFL \DIFaddFL{(mmol C m$^{-3}$)}\DIFaddendFL , alkalinity \DIFaddbeginFL \DIFaddFL{(meq m$^{-3}$) }\DIFaddendFL and dissolved oxygen \DIFaddbeginFL \DIFaddFL{(mmol O$_2$ m$^{-3}$)}\DIFaddendFL .}
\label{fig:carbon}
\end{figure}
...
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{$\Delta$pCO$_2$ \DIFdelbeginFL \DIFdelFL{(ppm) }\DIFdelendFL - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/atm_dpCO2.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/atm_dpCO2.png}
 \DIFaddendFL \label{fig:dpCO2_1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{$\Delta$pCO$_2$ \DIFdelbeginFL \DIFdelFL{(ppm) }\DIFdelendFL - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/atm_dpCO2.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/atm_dpCO2.png}
 \DIFaddendFL \label{fig:dpCO2_2}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/atm_dpCO2_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/atm_dpCO2_clrbr.png}
\DIFaddendFL \end{subfigure}
\caption{(Preindustrial) surface $\Delta$pCO$_2$ \DIFaddbeginFL \DIFaddFL{(ppm)}\DIFaddendFL .}
\label{fig:dpCO2}
\end{figure}
...
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{POC production - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_POC.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/bio_fpart_POC.png}
 \DIFaddendFL \label{fig:dpCO2_1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{POC production - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/bio_fpart_POC.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POC.png}
 \DIFaddendFL \label{fig:dpCO2_2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{ECOGEM$\div$BIOGEM}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_POC_diff.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POC_diff.png}
 \DIFaddendFL \label{fig:dpCO2_2}
\end{subfigure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\[+0.2cm]
...
\begin{subfigure}{.49\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/bio_fpart_POC_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POC_clrbr.png}
\DIFaddendFL \end{subfigure}
\begin{subfigure}{.49\textwidth}
 \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_POC_clrbr_diff.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POC_clrbr_diff.png}
\DIFaddendFL \end{subfigure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\[+0.2cm]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{POP production - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_POP.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/bio_fpart_POP.png}
 \DIFaddendFL \label{fig:dpCO2_1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{POP production - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/bio_fpart_POP.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POP.png}
 \DIFaddendFL \label{fig:dpCO2_2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{ECOGEM$\div$BIOGEM}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_POP_diff.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POP_diff.png}
 \DIFaddendFL \label{fig:dpCO2_2}
\end{subfigure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\[+0.2cm]
...
\begin{subfigure}{.49\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/bio_fpart_POP_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POP_clrbr.png}
\DIFaddendFL \end{subfigure}
\begin{subfigure}{.49\textwidth}
 \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_POP_clrbr_diff.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POP_clrbr_diff.png}
\DIFaddendFL \end{subfigure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\[+0.2cm]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{POFe production - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_POFe.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/bio_fpart_POFe.png}
 \DIFaddendFL \label{fig:dpCO2_1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{POFe production - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/bio_fpart_POFe.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POFe.png}
 \DIFaddendFL \label{fig:dpCO2_2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{ECOGEM$\div$BIOGEM}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_POFe_diff.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POFe_diff.png}
 \DIFaddendFL \label{fig:dpCO2_2}
\end{subfigure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\[+0.2cm]
...
\begin{subfigure}{.49\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/bio_fpart_POFe_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POFe_clrbr.png}
\DIFaddendFL \end{subfigure}
\begin{subfigure}{.49\textwidth}
 \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_POFe_clrbr_diff.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_POFe_clrbr_diff.png}
\DIFaddendFL \end{subfigure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\[+0.2cm]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{CaCO$_3$ production - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_CaCO3.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/bio_fpart_CaCO3.png}
 \DIFaddendFL \label{fig:dpCO2_1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{CaCO$_3$ production - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/bio_fpart_CaCO3.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_CaCO3.png}
 \DIFaddendFL \label{fig:dpCO2_2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{ECOGEM$\div$BIOGEM}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_CaCO3_diff.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_CaCO3_diff.png}
 \DIFaddendFL \label{fig:dpCO2_2}
\end{subfigure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\[+0.2cm]
...
\begin{subfigure}{.49\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/bio_fpart_CaCO3_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_CaCO3_clrbr.png}
\DIFaddendFL \end{subfigure}
\begin{subfigure}{.49\textwidth}
 \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/bio_fpart_CaCO3_clrbr_diff.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/bio_fpart_CaCO3_clrbr_diff.png}
\DIFaddendFL \end{subfigure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\caption{\DIFdelbeginFL \DIFdelFL{Particulate matter production }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Vertical fluxes of particulate carbon }\DIFaddendFL (\DIFaddbeginFL \DIFaddFL{mmol C m$^{-2}$ d$^{-1}$), phosphorus (mmol P m$^{-2}$ d$^{-1}$), iron (mmol Fe m$^{-2}$ d$^{-1}$) }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{export from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{calcium carbonate (mmol CaCO$_3$ m$^{-2}$ d$^{-1}$) across }\DIFaddendFL the \DIFaddbeginFL \DIFaddFL{base of the }\DIFaddendFL surface layer\DIFdelbeginFL \DIFdelFL{)}\DIFdelendFL . The right-hand column indicates the relative increase or decrease in ECOGEM, relative to BIOGEM \DIFaddbeginFL \DIFaddFL{(dimensionless)}\DIFaddendFL .}
...
In this section we present the meridional depth distributions of key biogeochemical tracers, averaged across each of the three main ocean basins, as shown in Figure~\ref{fig:basins}. Figure~\ref{fig:basin_PO4} shows that the distribution of dissolved phosphate is very similar between the two models, with \DIFdelbegin \DIFdel{EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE }\DIFaddend showing a slightly stronger sub-surface accumulation in the northern Indian Ocean. 

The vertical distributions shown in Figure~\ref{fig:basin_Fe} reveal that dissolved iron is lower throughout the ocean in \DIFdelbegin \DIFdel{EcoGENIE}\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE}\DIFaddend , relative to \DIFdelbegin \DIFdel{cGENIE}\DIFdelend \DIFaddbegin \DIFadd{cGEnIE}\DIFaddend , particularly below 1500 m. Differences are less obvious at intermediate depths. (Observations are currently too sparse to estimate reliable basin-scale distributions of dissolved iron; see Tagliabue 2016.)

Figure~\ref{fig:basin_DIC} shows that while \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend reproduces observed DIC distributions very well, \DIFdelbegin \DIFdel{EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE }\DIFaddend overestimates concentrations within the Indian and Pacific Oceans. The total oceanic DIC inventory increased by just under 2\% from \DIFdelbegin \DIFdel{0.299 mol C in cGENIE to 0.304 in EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{2.99 Examol C in cGEnIE to 3.05 in EcoGEnIE }\DIFaddend (with a fixed atmospheric CO\(_{2}\) concentration of 278 ppm). Otherwise the two models show broadly similar distributions, with the most pronounced differences (as for PO$_4$) in the northern Indian Ocean.

Figure~\ref{fig:basin_O2} shows that \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend reasonably captures the invasion of O$_2$ into the ocean interior through the Southern Ocean and North Atlantic. These patterns are also seen in \DIFdelbegin \DIFdel{EcoGENIE}\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE}\DIFaddend , although unrealistic water column anoxia is seen in the northern intermediate Indian and Pacific Oceans. Again, this is likely a consequence of greater export and remineralisation of organic carbon in \DIFdelbegin \DIFdel{EcoGENIE}\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE}\DIFaddend , leading to more oxygen consumption at intermediate depths (also evidenced by elevated PO$_4$, DIC and alkalinity in the same regions; Figures~\ref{fig:basin_PO4}, \ref{fig:basin_DIC} and~\ref{fig:basin_ALK}).

Alkalinity (Figure~\ref{fig:basin_ALK}) also shows some clear differences between the two models, again most noticeably in the northern intermediate Indian and Pacific Oceans. In these regions \DIFdelbegin \DIFdel{EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE }\DIFaddend shows excessive accumulation of alkalinity at $\sim$1000~m depth. This is again attributable to the increased C export in \DIFdelbegin \DIFdel{EcoGENIE}\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE}\DIFaddend . In the absence of a nitrogen cycle (and NO\(^{-}_{3}\) reduction), increased anoxic remineralisation of organic carbon (Figures~\ref{fig:basin_DIC} and~\ref{fig:basin_O2}) leads to increased reduction of sulphate to H\(_{2}\)S, which in turn increases the alkalinity of seawater. Further adjustment of the \DIFdelbegin \DIFdel{respiration of carbon }\DIFdelend \DIFaddbegin \DIFadd{cellular nutrient quotas }\DIFaddend in ECOGEM and hence the effective exported P:C Redfield ratio, and/or retuning of the organic matter remineralisation profiles in BIOGEM \citep{Ridgwell:2007} would likely resolve these issues.


\begin{figure}[htp]
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.5\linewidth]{/Volumes/cGENIE/cgenie_output/EXAMPLE.worjh2.PO4FeTDTL_HYBRID.SPIN/figures/3basins.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.5\linewidth]{/Users/ward/cgenie_output/3basins.png}
...
\begin{subfigure}{.33\textwidth}
 ...
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/WOA_2009_data/Atlantic_p_an_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/WOA_2009_data/Atlantic_p_an_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic PO$_4$ - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Atlantic_ocn_PO4_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Atlantic_ocn_PO4_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic PO$_4$ - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Atlantic_ocn_PO4_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Atlantic_ocn_PO4_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian PO$_4$ - WOA (2009)}
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/WOA_2009_data/Indian_p_an_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/WOA_2009_data/Indian_p_an_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian PO$_4$ - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Indian_ocn_PO4_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Indian_ocn_PO4_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian PO$_4$ - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Indian_ocn_PO4_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Indian_ocn_PO4_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific PO$_4$ - WOA (2009)}
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/WOA_2009_data/Pacific_p_an_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/WOA_2009_data/Pacific_p_an_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific PO$_4$ - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Pacific_ocn_PO4_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Pacific_ocn_PO4_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific PO$_4$ - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Pacific_ocn_PO4_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Pacific_ocn_PO4_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_PO4_profile_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_PO4_profile_clrbr.png}
\DIFaddendFL \end{subfigure}
\caption{Basin-averaged meridional-depth distribution of phosphate \DIFaddbeginFL \DIFaddFL{(mmol P m$^{-3}$)}\DIFaddendFL .}
\label{fig:basin_PO4}
\end{figure}
...
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic dFe - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Atlantic_ocn_TDFe_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Atlantic_ocn_TDFe_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic dFe - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Atlantic_ocn_TDFe_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Atlantic_ocn_TDFe_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian dFe - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Indian_ocn_TDFe_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Indian_ocn_TDFe_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian dFe - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Indian_ocn_TDFe_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Indian_ocn_TDFe_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific dFe - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Pacific_ocn_TDFe_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Pacific_ocn_TDFe_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific dFe - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Pacific_ocn_TDFe_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Pacific_ocn_TDFe_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_TDFe_profile_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_TDFe_profile_clrbr.png}
\DIFaddendFL \end{subfigure}
\caption{Basin-averaged meridional-depth distribution of total dissolved iron (\DIFaddbeginFL \DIFaddFL{mmol }\DIFaddendFL dFe \DIFaddbeginFL \DIFaddFL{m$^{-3}$}\DIFaddendFL ).}
\label{fig:basin_Fe}
\end{figure}
...
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic DIC - \DIFdelbeginFL \DIFdelFL{GLODAP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GLODAPv2}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/GLODAP_data/Atlantic_TCO2_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/GLODAP_data/Atlantic_TCO2_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic DIC - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Atlantic_ocn_DIC_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Atlantic_ocn_DIC_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic DIC - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Atlantic_ocn_DIC_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Atlantic_ocn_DIC_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian DIC - \DIFdelbeginFL \DIFdelFL{GLODAP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GLODAPv2}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/GLODAP_data/Indian_TCO2_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/GLODAP_data/Indian_TCO2_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian DIC - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Indian_ocn_DIC_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Indian_ocn_DIC_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian DIC - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Indian_ocn_DIC_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Indian_ocn_DIC_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific DIC - \DIFdelbeginFL \DIFdelFL{GLODAP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GLODAPv2}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/GLODAP_data/Pacific_TCO2_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/GLODAP_data/Pacific_TCO2_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific DIC - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Pacific_ocn_DIC_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Pacific_ocn_DIC_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific DIC - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Pacific_ocn_DIC_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Pacific_ocn_DIC_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_DIC_profile_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_DIC_profile_clrbr.png}
\DIFaddendFL \end{subfigure}
\caption{Basin-averaged meridional-depth distribution of DIC \DIFaddbeginFL \DIFaddFL{(mmol C m$^{-3}$)}\DIFaddendFL .}
\label{fig:basin_DIC}
\end{figure}
...
\begin{subfigure}{.33\textwidth}
 ...
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/WOA_2009_data/Atlantic_o_an_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/WOA_2009_data/Atlantic_o_an_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic O$_2$ - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Atlantic_ocn_O2_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Atlantic_ocn_O2_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic O$_2$ - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Atlantic_ocn_O2_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Atlantic_ocn_O2_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian O$_2$ - WOA (2009)}
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/WOA_2009_data/Indian_o_an_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/WOA_2009_data/Indian_o_an_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian O$_2$ - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Indian_ocn_O2_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Indian_ocn_O2_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian O$_2$ - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Indian_ocn_O2_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Indian_ocn_O2_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific O$_2$ - WOA (2009)}
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/WOA_2009_data/Pacific_o_an_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/WOA_2009_data/Pacific_o_an_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific O$_2$ - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Pacific_ocn_O2_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Pacific_ocn_O2_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific O$_2$ - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Pacific_ocn_O2_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Pacific_ocn_O2_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_O2_profile_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_O2_profile_clrbr.png}
\DIFaddendFL \end{subfigure}
\caption{Basin-averaged meridional-depth distribution of dissolved oxygen \DIFaddbeginFL \DIFaddFL{(mmol O$_2$ m$^{-3}$)}\DIFaddendFL .}
\label{fig:basin_O2}
\end{figure}
...
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic ALK - \DIFdelbeginFL \DIFdelFL{GLODAP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GLODAPv2}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/GLODAP_data/Atlantic_PALK_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/GLODAP_data/Atlantic_TALK_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic Alkalinity - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Atlantic_ocn_ALK_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Atlantic_ocn_ALK_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Atlantic Alkalinity - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Atlantic_ocn_ALK_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Atlantic_ocn_ALK_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian ALK - \DIFdelbeginFL \DIFdelFL{GLODAP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GLODAPv2}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/GLODAP_data/Indian_PALK_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/GLODAP_data/Indian_TALK_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian Alkalinity - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Indian_ocn_ALK_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Indian_ocn_ALK_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Indian Alkalinity - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Indian_ocn_ALK_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Indian_ocn_ALK_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific ALK - \DIFdelbeginFL \DIFdelFL{GLODAP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GLODAPv2}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/GLODAP_data/Pacific_PALK_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/GLODAP_data/Pacific_TALK_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific Alkalinity - \DIFdelbeginFL \DIFdelFL{cGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new.SPIN/figures/Pacific_ocn_ALK_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_new_MLD.SPIN/figures/Pacific_ocn_ALK_profile.png}
 \DIFaddendFL \label{fig:nutrients1}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
 \centering
 \caption{Pacific Alkalinity - \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL }
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Pacific_ocn_ALK_profile.png}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Pacific_ocn_ALK_profile.png}
 \DIFaddendFL \label{fig:nutrients2}
\end{subfigure}
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/ocn_ALK_profile_clrbr.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/ocn_ALK_profile_clrbr.png}
\DIFaddendFL \end{subfigure}
\caption{Basin-averaged meridional-depth distribution of alkalinity \DIFaddbeginFL \DIFaddFL{(meq m$^{-3}$)}\DIFaddendFL .}
\label{fig:basin_ALK}
\end{figure}
...
Figures~\ref{fig:jgofs_po4} and~\ref{fig:jgofs_fe} \DIFdelbegin \DIFdel{we }\DIFdelend compare the seasonal cycles of surface nutrients (phosphate and iron) at nine Joint Global Ocean Flux Study (JGOFS) sites.
 \begin{figure}[htbp]
\begin{center}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=16cm]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_final.SPIN/figures/JGOFS_Phosphate.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=16cm]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/JGOFS_Phosphate.png}
\DIFaddendFL \caption{Annual cycle of surface PO$_4$ at 9 time-series sites in \DIFdelbeginFL \DIFdelFL{cGENIE }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{EcoGENIE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE}\DIFaddendFL . Red dots indicate climatological observations, while the lines represent modelled surface PO$_4$ concentrations. Locations of the time-series are indicated in Figure~\ref{fig:basins}.}
\label{fig:jgofs_po4}
\end{center}
\end{figure}
...
\begin{figure}[htbp]
\begin{center}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=16cm]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_final.SPIN/figures/JGOFS_Iron.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=16cm]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/JGOFS_Iron.png}
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/Satellite_data/SeaWiFS_on_GEnIE.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/Satellite_data/SeaWiFS_on_GEnIE.png}
\DIFaddendFL \end{subfigure}%
\begin{subfigure}{.5\textwidth}
\centering
 \caption{ECOGEM Chlorophyll}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Surface_Chl_Biomass.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Surface_Chl_Biomass.png}
\DIFaddendFL \end{subfigure}%
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/Surface_Chl_Biomass_clrbar.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/Surface_Chl_Biomass_clrbar.png}
\DIFaddendFL \end{subfigure}
%%%%%
\begin{subfigure}{.5\textwidth}
\centering
 \caption{\citet{Yool:2013a} Pr. Production}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/Satellite_data/Composite_PP_on_GEnIE.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/Satellite_data/Composite_PP_on_GEnIE.png}
\DIFaddendFL \end{subfigure}%
\begin{subfigure}{.5\textwidth}
\centering
 \caption{ECOGEM Pr. Production}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/PrimaryProdn.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/PrimaryProdn.png}
\DIFaddendFL \end{subfigure}%
\\[+0.2cm]
\begin{subfigure}{.5\textwidth}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/PrimaryProdn_clrbar.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/PrimaryProdn_clrbar.png}
\DIFaddendFL \end{subfigure}
...
\begin{subfigure}{.66\textwidth}
 \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/SizeClass_C_Biomass.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/SizeClass_C_Biomass.png}
\DIFaddendFL \end{subfigure} 
\\[+0.2cm]
\begin{subfigure}{.66\textwidth}
 \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/SizeClass_C_Biomass_clrbar.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/SizeClass_C_Biomass_clrbar.png}
\DIFaddendFL \end{subfigure}
\caption{Surface concentrations of carbon biomass in each population \DIFaddbeginFL \DIFaddFL{(mmol C m$^{-3}$)}\DIFaddendFL .}
\label{fig:surf_sizefrac}
\end{figure}

The degree of nutrient limitation within each phytoplankton size class is shown in Figure~\ref{fig:surf_sizefrac_limitation}. The two-dimensional colour-scale indicates decreasing iron limitation from left to right, and decreasing phosphorus limitation from bottom to top. White is therefore nutrient replete, blue is phosphorus limited, red is iron limited, and magenta is phosphorus-iron co-limited. The figure demonstrates that the smallest size class is not nutrient limited in any region. The increasing saturation of the colour scale in larger size classes indicates an increasing degree of nutrient limitation. As expected, nutrient limitation is strongest in the highly stratified low latitudes. A stronger vertical supply of nutrients at higher latitudes is associated with weaker nutrient limitation, although nutrient limitation is still significant among the larger size classes. Consistent with observations \citep{Moore:2013}, phosphorus limitation is restricted to low latitudes. Iron limitation dominates in high latitude regions\DIFdelbegin \DIFdel{. Among the }\DIFdelend \DIFaddbegin \DIFadd{, especially among }\DIFaddend larger size classes\DIFaddbegin \DIFadd{. Among these larger groups, }\DIFaddend the upwelling zones appear to be characterised by iron-phosphorus co-limitation.

\begin{figure}[htbp]
\begin{subfigure}{.33\textwidth}
 \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/SizeClass_Limitation.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/SizeClass_Limitation.png}
\DIFaddendFL \end{subfigure}
\begin{subfigure}{.25\textwidth}
 \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_new.SPIN/figures/SizeClass_Limitation_clrbar.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/SizeClass_Limitation_clrbar.png}
\DIFaddendFL \end{subfigure}
\caption{Nutrient limitation in each phytoplankton population \DIFaddbeginFL \DIFaddFL{(dimensionless)}\DIFaddendFL . The two-dimensional colour-scale indicates decreasing phosphorus limitation from left to right, and decreasing iron limitation from bottom to top. White is therefore nutrient replete, blue is phosphorus limited, red is iron limited, and magenta is phosphorus-iron co-limited.}
...
The seasonal cycles of phytoplankton chlorophyll~\textit{a} are compared to time-series observations in Figure~\ref{fig:jgofs_chl}. The modelled total chlorophyll concentrations (black lines) track the observed concentrations (red dots) reasonably well at most sites\DIFdelbegin \DIFdel{, and perhaps better than might be expected from the comparison to satellite data }\DIFdelend \DIFaddbegin \DIFadd{. The bottom three panels also suggest that the satellite data shown }\DIFaddend in Figure~\ref{fig:chl_prpr} \DIFaddbegin \DIFadd{may slightly underestimate surface chlorophyll concentrations in the Southern Ocean}\DIFaddend . The modelled surface chlorophyll concentration is \DIFdelbegin \DIFdel{probably }\DIFdelend too low in the equatorial Pacific, while the spring bloom occurs one to two months earlier than was seen during the North Atlantic Bloom Experiment. 

\begin{figure}[htbp]
\begin{center}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=16cm]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_final.SPIN/figures/JGOFS_Chlorophyll.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=16cm]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/JGOFS_Chlorophyll.png}
\DIFaddendFL \caption{Annual cycle of surface chlorophyll~\textit{a} at nine JGOFS time-series sites. Red dots indicate climatological observations, while the black lines represents modelled total surface chlorophyll~\textit{a}. Coloured lines represent chlorophyll~\textit{a} in individual size classes (blue = small, red = large). Locations of the time-series are indicated in Figure~\ref{fig:basins}. \DIFaddbeginFL \DIFaddFL{Satellite estimates of chlorophyll~}\textit{\DIFaddFL{a}} \DIFaddFL{are shown in grey.}\DIFaddendFL }
\label{fig:jgofs_chl}
\end{center}
\end{figure}

\DIFaddbegin \DIFadd{The seasonal cycles of primary production in the surface layer are compared to time-series observations in Figure~\ref{fig:jgofs_prpr}. As also indicated in Figure~\ref{fig:chl_prpr}, the spatial variance in modelled primary production is too low, with primary production overestimated at the most oligotrophic site (HOT) and typically underestimated at the most productive sites (esp. the equatorial Pacific, NABE and the Ross Sea). In contrast to the lack of spatial variability, the model exhibits significant seasonal variation, often in excess of the observed variability (at those sites where the seasonal cycle is well resolved).
}

\DIFaddend \begin{figure}[htbp]
\begin{center}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=16cm]{/Volumes/cGENIE/cgenie_output/BENW.worjh2.PO4FeTDTL_HYBRID_WF16_tgld_final.SPIN/figures/JGOFS_PrProduction.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=16cm]{/Users/ward/cgenie_output/BSS.8P8Z_noresp.SPIN/figures/JGOFS_PrProduction.png}
\DIFaddendFL \caption{Annual cycle of surface primary production at nine JGOFS time-series sites. Red dots indicate climatological observations, while the black lines represents modelled total primary production. Locations of the time-series are indicated in Figure~\ref{fig:basins}.}
\DIFdelbeginFL %DIFDELCMD < \label{fig:jgofs_chl}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:jgofs_prpr}
\DIFaddendFL \end{center}
\end{figure}

\clearpage
\subsubsection{\DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend vs. \DIFdelbegin \DIFdel{EcoGENIE}\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE}\DIFaddend }

Figure~\ref{fig:taylor} is a Taylor diagram comparing the two models in terms of their correlation to observations and their standard deviations, relative to observations. A perfect model would be located at the middle of the bottom axis, with a correlation coefficient of 1.0 and a normalised standard deviation of 1.0. The closer a model is to this ideal point, the better a representation of the data it provides. Figure~\ref{fig:taylor} shows that \DIFdelbegin \DIFdel{EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE }\DIFaddend is located further from the ideal point than \DIFdelbegin \DIFdel{cGENIE}\DIFdelend \DIFaddbegin \DIFadd{cGEnIE}\DIFaddend , in terms of oxygen, alkalinity, phosphate, and DIC. The new model seems to provide a universally worse representation of global ocean biogeochemistry. This is perhaps not surprising, given that the BIOGEM component of \DIFdelbegin \DIFdel{cGENIE }\DIFdelend \DIFaddbegin \DIFadd{cGEnIE }\DIFaddend has at various times been systematically tuned to match the observation data \citep[e.g.][]{Ridgwell:2007,Ridgwell:prep}. \DIFdelbegin \DIFdel{EcoGENIE }\DIFdelend \DIFaddbegin \DIFadd{EcoGEnIE }\DIFaddend has not yet been optimised in this way.

\begin{figure}[htbp]
\begin{center}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=8cm]{/Users/ward/GitHub/EcoGEnIE-desc/Figures/Taylor.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=10cm]{/Users/ward/GitHub/EcoGEnIE-desc/Figures/Taylor.pdf}
\DIFaddendFL \caption{Taylor diagram comparing \DIFdelbeginFL \DIFdelFL{cGENIE }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cGEnIE (white dots) }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{EcoGENIE }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EcoGEnIE (grey dots) }\DIFaddendFL to annual mean observation fields.}
\label{fig:taylor}
\end{center}
\end{figure}
...
This work was supported by the European Research Council `PALEOGENiE' project (ERC-2013-CoG-617313). BAW thanks the Marine Systems Modelling group at the National Oceanography Centre, Southampton\DIFaddbegin \DIFadd{. Satellite ocean colour data (Sea-viewing Wide Field-of-view Sensor }[\DIFadd{SeaWiFS}]\DIFadd{) were obtained from the National Aeronautics and Space Administration (NASA) Goddard Space Flight Center}\DIFaddend .
