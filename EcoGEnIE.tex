...
are taken directly from laboratory culture experiments of isolated strains [Le Quéré et al., 2005] creating a parameter-dependence on the modern ecosystem in addition to a structural one. 
...
Transformations and spatial redistribution of biogeochemical compounds both at the ocean surface (by biological uptake) and in the ocean interior (remineralziation), plus air-sea gas exchange, are handled by the module BIOGEM. In the current version of the ecosystem scheme in BIOGEM, in place of an explicit representation of living microbial community, the biological (soft-tissue) pump is driven by an implicit (i.e. unresolved) biological community -- a nutrient limitation variant of a model of biogenically induced chemical fluxes, as outlined earlier. A full description can be found in \citep{Ridgwell:2007, Ridgwell:prep}. 

In this study, we use a seasonally insolation forced, 16-level ocean model based configuration, similar to that of \citet{Cao:2009}. However, in the particular biogeochemical configuration we use, limitation of biological uptake of carbon is provided by the availability of two nutrients and in addition to phosphate, we now include an iron cycle following \citep{Tagliabue:2016}. To this, we make use a revised set of parameters controlling the iron cycle. We also incorporate a series of minor modifications to the climate model component, particularly in terms of the ocean grid and wind velocity and stress forcings \citep[consistent with][]{Marsh:2011} together with associated changes to several of the physics parameters. A complete description and evaluation of the physical and biogeochemical configuration of cGENIE is provided in \citep{Ridgwell:prep}. 
...
\begin{itemize}
\item[$\bullet$] surface inorganic nutrients $\xrightarrow[\rm and~export]{\rm production}$ DOM + POM
\end{itemize}

\noindent This simplification greatly facilitates the efficient modelling of the carbon cycle over long time scales, but with the associated inherent caveats of a nutrient-limitation type scheme of induced fluxes (discussed earlier). In ECOGEM, biological uptake is again limited by light, temperature and nutrient availability, but here it must pass through an explicit and dynamic intermediary plankton biomass pool, before being returned to DOM or dissolved inorganic nutrients:


\begin{itemize}
\item[$\bullet$] {surface inorganic nutrients} $\xrightarrow[]{\rm production}$ {plankton biomass} $\xrightarrow[]{\rm export}$ {DOM + POM}
\end{itemize}
...
$\matr{R}$ is a row vector of length $I_r$, the number of dissolved inorganic nutrient species.
%
\begin{equation}
...
The photosynthesis model is modified from \cite{Geider:1998} and \cite{Moore:2002}. Light-limitation is calculated as a Poisson function of local irradiance ($I$), modified by the iron-dependent initial slope of the P-I curve ($\alpha \gamma_{j,{\rm Fe}}$) and the chlorophyll-\textit{a}-to-carbon ratio ($Q_{j,{\rm Chl}}$). 
%
\begin{equation}
\label{gammaI}
\gamma_{j,I} = \Big[ 1- \exp \Big( \frac{-\alpha \cdot \gamma_{j,{\rm Fe}} Q_{j,{\rm Chl}} \cdot { I}}{ P_{j,{\rm C}}^{\rm sat}} \Big) \Big]
\end{equation}
%
...
%
\begin{equation}
V_{j,{\rm C}} = P_{j,{\rm C}} - \xi \cdot 16V_{j,{\rm P}}
\end{equation}
%
...
%
\begin{equation}
\rho_{j,{\rm Chl}} = \theta_{\rm N}^{\rm max} \frac{P_{j,{\rm C}}}{\alpha \cdot \gamma_{j,{\rm Fe}} \cdot Q_{j,{\rm Chl}} \cdot {I}}
\end{equation}
%
where $\rho_{j,{\rm Chl}}$ is the amount of chlorophyll \textit{a} that is synthesised for every mmol of nitrogen assimilated (mg Chl (mmol N)$^{-1}$). If phosphorus is assimilated at carbon specific rate $V_{j,{\rm P}}$ (mmol N (mmol C)$^{-1}$ d$^{-1}$), then the carbon specific rate of chlorophyll \textit{a} synthesis (mg chl (mmol C)$^{-1}$ d$^{-1}$) is
%
\begin{equation}
V_{j,{\rm Chl}} = \rho_{j,{\rm Chl}} \cdot 16V_{j,{\rm P}}
\label{chl_synthesis}
\end{equation}
%
%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
\gamma_{\rm T}
\cdot \underbrace{\frac{G_{j_{\rm pred},{\rm C}}^{\rm max} \cdot \alpha_{j_{\rm pred},{\rm C}} \cdot \mathcal{F}_{j_{\rm pred},{\rm C}}}{G_{j_{\rm pred},{\rm C}}^{\rm max} + \alpha_{j_{\rm pred},{\rm C}}\cdot \mathcal{F}_{j_{\rm pred},{\rm C}} } }_{\rm overall~grazing~rate}
...
\end{equation}
%
Finally, a prey refuge function is incorporated, such that the overall grazing rate is reduced when the availability of all prey ($\mathcal{F}_{j_{\rm pred},{\rm C}}$) is low. The size of the prey refuge is dictated by the coefficient $\Lambda$. The overall grazing response is calculated on the basis of prey carbon. Grazing losses of other prey elements are simply calculated from their stoichiometric ratio to prey carbon, with different elements assimilated according to the predator's nutritional requirements (see below).
%
\begin{equation}
...
\end{equation}
%
If all three quotas are full, C is assimilated at the maximum rate. If any are empty, C assimilation is down-regulated until sufficient quantities of the limiting element(s) are acquired.
%
\begin{equation}
...
%
\begin{equation}
m_j = m_p (1-e^{-10^{10}\matr{B}_{j,C}})
\end{equation}
%
...
%
\begin{equation} 
V_{j,O_2} = -\frac{106}{138} V_{j,{\rm DIC}} B_{j,{\rm C}}
\end{equation}
%
...
\end{equation}
%
The negative sign indicates that alkalinity increases as PO$_4$ is consumed. This relationship accounts for alkalinity changes associated with N transformations \citep{Zeebe:2001} that are not explicitly represented in this particular biogeochemical configuration of cGENIE (because no N species in the ocean are defined).

%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
\subsubsection{Production of organic matter}

Plankton mortality and grazing are the only two sources of organic matter, with partitioning between non-sinking dissolved and sinking particulate phases determined by the parameter $\beta_j$. In this initial implementation of ECOGEM, for traceability, the assumptions are the same as made in the current version of BIOGEM [\textcolor{red}{REFS}] which themselves follow the OCMIP2 ocean carbon cycle modelling intercomparison protocol described in \citet{Najjar:2007}. Specifically, $\beta_j$ is set to a fixed fraction $\beta$ for all size classes. 


%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
\subsection{Differential equations}\label{PDEs}

Differential equations for ${\matr R}$, ${\matr B}$ and ${\matr D}$ are written below. The dimensions of each matrix and vector used equations~\ref{dRdt} to~\ref{dDdt} are given in Table~\ref{matrix_table}. Note that while $\matr{R}$ and $\matr{OM}$ are transported by the physical component of GENIE, living biomass $\matr{B}$ is not currently subject to any physical transport. The only communication between biological communities in adjacent grid cells is through the advection and diffusion of inorganic resources and non-living organic matter in BIOGEM. Note that some additional sources and sinks of $\matr{R}$, and all sinks of $\matr{D}$, are computed in BIOGEM.
...
%
\begin{align}
\frac{\partial {R_{i_r}}}{\partial t} = & \sum_{j=1}^{J} \Big( \underbrace{-V_{j,i_r} \cdot B_{j,{\rm C}} }_{\rm uptake}
 + \underbrace{r_{j,i_r} \cdot B_{j,{\rm C}} }_{\rm respiration} \Big) \label{dRdt}
\end{align}
%
...
\frac{\partial B_{j,i_b}}{\partial t} = &+ \underbrace{V_{j,i_b} \cdot B_{j,C}}_{\rm uptake} 
 ...
 - \underbrace{r_{i_b,j} \cdot B_{C,j}}_{\rm respiration} \nonumber\\ 
 &+ \underbrace{B_{j,{\rm C}} \cdot \lambda_{j,i_b} \sum_{j_{\rm prey}=1}^{J}G_{j,j_{\rm prey},i_b} }_{\rm grazing~gains} 
...
\frac{\partial {\rm D}_{1,i_d}}{\partial t} =& \underbrace{ \sum_{j=1}^{J} [{\rm B}_{j,i_d}] \beta_{j} m_j }_{\rm mortality} 
 + \underbrace{ \sum_{j_{\rm pred}=1}^{J} [{\rm B}_{j_{\rm pred},{\rm C}}] \sum_{j_{\rm prey}=1}^{J} \beta_{j_{\rm prey}} (1-\lambda_{j_{\rm pred},i_b}) G_{j_{\rm pred},j_{\rm prey},i_d}}_{\rm messy~feeding} \label{dDdt}
\end{align}
%
Dissolved organic matter ($k=1$) is an explicit tracer that is transported by the ocean circulation model and is degraded back to its constituent nutrients with a fixed turnover time of $\lambda$ (= 0.5 years). The production of particulate organic matter (POM) in the surface layer is given by 
%
\begin{equation} 
F_{z=z_{0}}^{\text{POM}} = \underbrace{ \sum_{j=1}^{J} [{\rm B}_{j,i_d}] (1-\beta_{j}) m_j}_{\rm mortality} + \underbrace{ \sum_{j_{\rm pred}=1}^{J} [{\rm B}_{j_{\rm pred},{\rm C}}] \sum_{j_{\rm prey}=1}^{J} (1-\beta_{j_{\rm prey}}) (1-\lambda_{j_{\rm pred},i_b}) G_{j_{\rm pred},j_{\rm prey},i_d}}_{\rm messy~feeding}
\end{equation} 
% 
However, POM is not represented as an explicit state variable in either ECOGEM or BIOGEM. Instead, surface production is redistributed throughout the water column as a depth dependent flux, $F_z^{\text{POM}}$. To achieve this, $F_{z=z_{0}}^{\text{POM}}$ is partitioned between a `refractory' component ($r^{\text{POM}}$) that is predominantly remineralised in the deep ocean at the seafloor and a `labile' component ($1-r^{\text{POM}}$) which predominantly remineralises in the upper water column. The net remineralisation at depth $z$, relative to the export depth $z_{0}$ is determined by characteristic length scales (${l^{\text{rPOM}}}$ and ${l^{\text{POM}}}$ for `refractory' and `labile' POM respectively):
%
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=8cm]{/Users/ward/GitHub/EcoGEnIE-desc/Figures/Taylor.png}
